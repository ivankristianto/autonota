...
**Goal:** Add `npm run release` that bumps version, runs quality gates, publishes to npm, and pushes a git tag.

**Architecture:** A single bash script (`scripts/release.sh`) orchestrated via an npm script. The script handles pre-flight checks, version bumping with `jq`, quality gate execution, git tagging, dry-run + confirmed publish, and push.

**Tech Stack:** Bash, jq, npm, git
...
- [ ] **Step 1: Create the scripts directory and release.sh**

```bash
mkdir -p scripts
```

Create `scripts/release.sh` with the full implementation:
...
In `package.json`, add the `release` script to the `"scripts"` section:
...
"scripts": {
  ...
  "lint": "npm run format:check && eslint src tests",
  "test": "vitest run",
...
git commit -m "feat: add npm run release script for semver publish workflow"
```
